...
init-vcs:
	picker export --autobuild=false $(RTL_DIR)/$(TOP_MODULE).sv -w $(PWD)/$(TOP_MODULE).fsdb --sname $(TOP_MODULE) --tdir $(AXI2UI_DIR) --lang python -e -c --sim vcs --internal $(INTERNAL_FILE)
# $(MAKE) -C $(AXI2UI_DIR)
