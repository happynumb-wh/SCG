...
module mobile_ddr3 (
    ...
    reg                       cke_in;
    reg                       cs_n_in;
    reg     [CA_BITS-1:0]     ca_in;
    reg     [DM_BITS-1:0]     dm_in;
    reg     [DQ_BITS-1:0]     dq_in;
    reg     [DQS_BITS-1:0]    dqs_in;
    reg     [DQS_BITS-1:0]    dqs_n_in;
    reg                         odt_in;
    // transmit
    reg                       dqs_out_en_dly;
    reg                       dqs_out_dly;
    ...
    reg                       dqs_out_ca_en_dly;
    reg      [DQS_BITS-1:0]   dqs_out_ca_dly;
    reg      [DQS_BITS-1:0]   dqs_out_ca_n_dly;
    ...
    // cke, ca_in[0], ca_in[1], ca_in[2], ca_in[[3]
   ...
   parameter MRRBIT = 1'b0;
    ...
    //  Added flags to disable certain checks.
    reg  check_clocks;
    ...
    // Added to simulate board delays
    ...
    `ifdef OVRD_TDQSCK
    real  rnd_tdqsck;  // extra read delay on DQS# strobe
    `endif
 
    // command address
    reg                         cke_q;
    wire                  [4:0] cmd = {cke_in, ~cs_n_in ? {ca_in[0], ca_in[1], ca_in[2], ca_in[3]} : 4'b111x};  // deselect = nop 
    reg                   [3:0] cke_cmd;
    ...
    // rx
    wire                  [7:0] dqs_even = dqs_in;
    wire                  [7:0] dqs_odd = dqs_n_in;
    ...
    bufif1 buf_dqs    [DQS_BITS-1:0] (dqs,    {DQS_BITS{dqs_out_dly}}, dqs_out_en_dly);
    bufif1 buf_dqs_n  [DQS_BITS-1:0] (dqs_n, {DQS_BITS{~dqs_out_dly}}, dqs_out_en_dly);
    ...
    bufif1 buf_dqs_ca    [DQS_BITS-1:0] (dqs,    dqs_out_ca_dly, dqs_out_ca_en_dly);
    bufif1 buf_dqs_n_ca  [DQS_BITS-1:0] (dqs_n,  dqs_out_ca_n_dly, dqs_out_ca_en_dly);
...
`endif
   ...
   always @(mr[42])
    ...
    `ifdef OVRD_TDQSCK
    `ifdef TDQSCK_VALUE
      // tDQSCK value passed in thru command line, use this
      rnd_tdqsck = `TDQSCK_VALUE;
    `else
      // create a randmoized tDQSCK
      rnd_tdqsck = $random%(TDQSCK_MAX-TDQSCK);
      if (rnd_tdqsck < 0)
        rnd_tdqsck = -rnd_tdqsck;
      rnd_tdqsck = TDQSCK+rnd_tdqsck; 
    `endif
     $display("[MOBILE_DDR3] rnd_tdqsck = %0d", rnd_tdqsck);
    `endif
    ...
    always @(ca     ) ca_in     <= #(BUS_DELAY+cmd_dly) ca;
    //always @(odt    ) if (!feature_odt_hi) odt_in    <= #(BUS_DELAY+cmd_dly) odt;
    always @(odt    ) odt_in    <= #(BUS_DELAY+cmd_dly) odt;
    //Write data lanes board delay
    always @(dm     ) dm_in     <= #(BUS_DELAY+wr_dly) dm;
    always @(dq     ) dq_in     <= #(BUS_DELAY+wr_dly) dq;
    always @(dqs    ) dqs_in    <= #(BUS_DELAY+wr_dly) dqs;
    always @(dqs_n  ) dqs_n_in  <= #(BUS_DELAY+wr_dly) dqs_n;
    //Read data lanes board delay
    always @(dqs_out_en) dqs_out_en_dly <= #(rd_dly+qs_dly) dqs_out_en;
    always @(dqs_out)    dqs_out_dly    <= #(rd_dly+qs_dly) dqs_out;
    always @(dq_out_en)  dq_out_en_dly  <= #(rd_dly) dq_out_en;
    always @(dq_out)     dq_out_dly     <= #(rd_dly) dq_out;
 
    always @(dqs_out_ca_en) dqs_out_ca_en_dly <= #(rd_dly+qs_dly) dqs_out_ca_en;
    always @(dqs_out_ca)    dqs_out_ca_dly    <= #(rd_dly+qs_dly) dqs_out_ca;
    always @(dqs_out_ca_n)    dqs_out_ca_n_dly    <= #(rd_dly+qs_dly) dqs_out_ca_n;
    ...
    begin
        casex ({fromcmd, cmd})
          ...
          //{REFPB_CMD, DPD_CMD  } , legal 
            {REFPB_CMD, SREF_CMD } : begin if (check_rfc_max && ($time - tm_ref < TRFCPB)) ERROR ("tRFCpb violation"); end
            {REFPB_CMD, ACT_CMD  } : begin if (check_rfc_max && ($time - tm_bank_ref[ba] < TRFCPB)) ERROR ("tRFCpb violation"); if ((ck_cntr - ck_ref < RRD) || ($time - tm_ref < TRRD)) ERROR ("tRRD violation"); end
          ...
          //{REFAB_CMD, DPD_CMD  } , legal 
            {REFAB_CMD, SREF_CMD } : begin if (check_rfc_max && ($time - tm_refa < TRFCAB)) ERROR ("tRFCab violation"); end
    ...
    always @(diff_ck) begin
        ...
        if ((init != 1) && (cke_in === 1'bx))
            ERROR ("cke must be driven to a defined logic level");
        if (cke_q && ~cs_n_in && ((&ca === 1'bx) || (|ca === 1'bx)))
            ERROR ("ca must be driven to a defined logic level while cke/cs_n_in is active");
            ...
            case (init)
                ...
                3 : begin // 2. Reset command
                    ...
                    if ((cmd[4:1] !== 4'b1111) && ($time - tm_init3 < TINIT3)) begin
                        WARN ("tINIT3 violation");
                    end
                    // reset moves to tINIT4
                    if ((cmd == MRW_CMD) && (ca_in[9:4] == 'h3F)) begin
                        init = 4;
                        tm_init4 <= $time;
                    end
...
`endif
	    ...
	    case (ca_train)
		 ...
		 // NOP allowed during tCACKEL
                 if ((cs_n_in !== 1'b1 || cke_in !== 1'b1) && (ck_cntr - ck_ca_train0 < TCACKEL)) begin
		 ...
		 // NOP allowed during tCAENT/tCAMRD
                 if ((cs_n_in !== 1'b1) && (ck_cntr - ck_ca_train1 < TCAENT)) begin
                    ERROR ("tCAENT violation");
                 end
                 if ((cs_n_in !== 1'b1) && (ck_cntr - ck_ca_train0 < TCAMRD)) begin
                    ERROR ("tCAMRD violation");
                 end
		 // CA Train cmd
		 if (cs_n_in === 1'b0) begin
		    // tCACD check
		 ...
		 if (cke_in === 1'b1) begin
	      ...
	      5: begin // CKE HIGH
                 if ((cs_n_in !== 1'b1 || cke_in !== 1'b1) && (ck_cntr - ck_ca_train3 < TCAEXT)) begin
	    ...
	    NOP_CMD: begin end
            default: begin
                ...
                case (calibration_code)
                    CAL_INIT  : begin if ($time - tm_zq < TZQINIT) WARN ("tZQINIT violation"); end
	    ...
	    endcase
            ...
            casex ({cke_q, cmd})
                {1'b1, MRW_CMD} : begin
                    if (|bank_active && ca_in[9:4] != 'h3F) begin // reset for DPDE
                        ERROR ("All banks must be Precharged prior to MRW");
                    end else begin
                        if (ca_in[9:4] == 'h3F) begin
                            INFO ("Reset");
`ifdef MAX_MEM
...
`endif
			    ...
			    mr[4] <= 8'h03;   // 8'h03 ignore refresh
...
`endif
	   ...
	   if (ca_neg_en) begin
		 ...
		 dq_out[1] <= #TADR ca_in[0];
		 dq_out[3] <= #TADR ca_in[1];
		 dq_out[5] <= #TADR ca_in[2];
		 dq_out[7] <= #TADR ca_in[3];
		 dq_out[9] <= #TADR ca_in[5];
		 dq_out[11] <= #TADR ca_in[6];
		 dq_out[13] <= #TADR ca_in[7];
		 dq_out[15] <= #TADR ca_in[8];
	      end
	      else begin // MODE 48
		 dq_out[0] <= #TADR ca_q[4];
		 dq_out[8] <= #TADR ca_q[9];
		 dq_out[1] <= #TADR ca_in[4];
		 dq_out[9] <= #TADR ca_in[9];
	      end
	      ...
	      ca_neg_en <= 1'b0;
           ...
           if (neg_en) begin
              ma = {ca_in[1:0], ca_q[9:4]};
              op = ca_in[9:2];
              ba = ca_q[9:7];
              r = {ca_in[9:8], ca_q[6:2], ca_in[7:0]};
              c = {ca_in[9:1], ca_q[6:5], 1'b0};
              ecbbl = {ca_q[3], ca_q[4]};
		      ...
		      // still output data for write only registers 
                ...
                3'b100 : begin // WRITE
                   ...
                   $sformat (msg, "WRITE bank %d col %h ap %d", ba, c, ca_in[0]);
                   INFO (msg);
		   bl = updatebl(ecbbl);
		   lastwr_bl = bl;
                   ...
                   bank_ap[ba] <= ca_in[0]; // AP
                   write_ap[ba] <= ca_in[0]; // AP
                end
                3'b101 : begin // READ
                   if (c >= 1<<COL_BITS) begin
                      $sformat (msg, "col = %h does not exist.  Maximum col = %h", c, (1<<COL_BITS)-1);
                   ...
                   $sformat (msg, "READ bank %d col %h ap %d", ba, c, ca_in[0]);
                   INFO (msg);
		   bl = updatebl(ecbbl);
		   lastrd_bl = bl;
                   for (i=0; i<bl/2; i=i+1) begin
        ...
        if (!ca_train && !writeleveling) begin
           ...
           if (diff_ck) begin
         `ifdef OVRD_TDQSCK
              odt_rd_dis <= #(rnd_tdqsck-300) (|{rd_pipeline[2:0],rd_pipeline_prevbit0});
              dqs_out_en <= #(rnd_tdqsck) (|rd_pipeline[1:0]);
              dq_out_en <= #(rnd_tdqsck) (rd_pipeline[0]); 
         `else
         ...
         `ifdef OVRD_TDQSCK
            dqs_out <= #(rnd_tdqsck) rd_pipeline[0] && diff_ck;
            dq_out <= #(rnd_tdqsck) dq_temp;
         `else
    ...
    task dqs_even_receiver;
	   ...
	   if (~dqs_even_prev[i] && dqs_even[i]) begin
		 ...
		 for(k=0; k<`DQ_PER_DQS; k++)
		   dq_out[k+i*`DQ_PER_DQS] <= #TWLO ck_in;
	      end
	   end
    ...
    // odt 
    always @(odt_in or rtt_mode_reg[1:0]) 
      if (odt_in===1'b1 && rtt_mode_reg[1:0])  begin
    ...
    task WARN;
        input [MSGLENGTH*8:1] msg;
        begin
            $fdisplay(mcd_warn, "%m at time %t WARNING: %0s", $time, msg);
            warnings = warnings + 1;
        end
    endtask
    ...
    task ERROR;
        input [MSGLENGTH*8:1] msg;
        begin
            $fdisplay(mcd_error, "%m at time %t ERROR: %0s", $time, msg);
            errors = errors + 1;
