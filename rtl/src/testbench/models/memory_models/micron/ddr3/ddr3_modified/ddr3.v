...
`include "ddr3_parameters.vh"
    ...
    reg  check_clocks;
    reg  check_dq_setup_hold;
    reg  check_dq_pulse_width;
    reg  check_dqs_ck_setup_hold;
    reg  check_rfc_max;
    reg  check_cmd_addr_timing;
    reg  check_ctrl_addr_pulse_width;
    reg  check_odth;
    reg  check_dqs_latch;
    reg  check_pd_max;
    event  e_tm_dqs_neg;
...
`endif
    ...
    real                         wr_dly;     // write board delay
    real                         rd_dly;     // read board delay
    real                         ck_dly;     // extra write delay on CK/CK#
    real                         qs_dly;     // extra read delay on strobes
    real                         dqs_dly;    // extra read delay on DQS strobe
    real                         dqs_b_dly;  // extra read delay on DQS# strobe

    reg                          gen_error;
    reg [7:0]                    gen_error_bitpos;
    reg [63:0]                   error_mask;
    reg [63:0]                   error_mask_ns;
    reg                          wl_feedback_on_all_bits;
    integer                      wl_feedback_on_bit;

    // board delays are zeros by default (testcases can change these values
    // through hierarchy to simulate different board delays
    initial
    begin
        cmd_dly   = 0.0;
        wr_dly    = 0.0;
        ...
        dqs_b_dly = 0.0;
        check_clocks = 1'b1;
        check_dq_setup_hold = 1'b1;
        check_dq_pulse_width = 1'b1;
        check_dqs_ck_setup_hold = 1'b1;
        check_rfc_max = 1'b1;
        check_cmd_addr_timing = 1'b1;
        check_ctrl_addr_pulse_width = 1'b1;
 `ifdef DRAM_NO_CHECK_ODTH
        check_odth = 1'b0;
 `else
        check_odth = 1'b1;
 `endif
        check_dqs_latch = 1'b1;
        check_pd_max = 1'b1;
        gen_error = 1'b0;
        gen_error_bitpos = 0;
        error_mask = {64{1'b0}};
        error_mask_ns = {64{1'b0}};
        wl_feedback_on_all_bits = 1'b0;
        wl_feedback_on_bit      = 0;
    end

    reg [DQS_BITS-1:0]     dqs_out_en_dly_bd;
    reg [DQS_BITS-1:0]     dqs_out_dly_bd;
    reg [DQ_BITS-1:0]      dq_out_en_dly_bd;
    reg [DQ_BITS-1:0]      dq_out_dly_bd;

    // added write board delay
    always @(rst_n  ) rst_n_in  <= #BUS_DELAY rst_n;
    always @(ck     ) ck_in     <= #(BUS_DELAY+cmd_dly) ck;
    always @(ck_n   ) ck_n_in   <= #(BUS_DELAY+cmd_dly) ck_n;
    always @(cke    ) cke_in    <= #(BUS_DELAY+cmd_dly) cke;
    always @(cs_n   ) cs_n_in   <= #(BUS_DELAY+cmd_dly) cs_n;
    always @(ras_n  ) ras_n_in  <= #(BUS_DELAY+cmd_dly) ras_n;
    always @(cas_n  ) cas_n_in  <= #(BUS_DELAY+cmd_dly) cas_n;
    always @(we_n   ) we_n_in   <= #(BUS_DELAY+cmd_dly) we_n;
    always @(dm_tdqs) dm_in     <= #(BUS_DELAY+wr_dly) dm_tdqs;
    always @(ba     ) ba_in     <= #(BUS_DELAY+cmd_dly) ba;
    always @(addr   ) addr_in   <= #(BUS_DELAY+cmd_dly) addr;
    always @(dq     ) dq_in     <= #(BUS_DELAY+wr_dly) dq;
    always @(dqs or dqs_n) dqs_in <=#(BUS_DELAY+wr_dly) (dqs_n<<32) | dqs;
    always @(odt    ) if (!feature_odt_hi) odt_in    <= #(BUS_DELAY+cmd_dly) odt;
    ...
    // output (read) board delays
    always @(dqs_out_en_dly) dqs_out_en_dly_bd <= #(rd_dly+qs_dly) dqs_out_en_dly;
    always @(dqs_out_dly)    dqs_out_dly_bd    <= #(rd_dly+qs_dly) dqs_out_dly;
    always @(dq_out_en_dly)  dq_out_en_dly_bd  <= #(rd_dly) dq_out_en_dly;
    always @(dq_out_dly)     dq_out_dly_bd     <= #(rd_dly) dq_out_dly;

    // use signals with output delays (original code commented out)
    bufif1 buf_dqs    [DQS_BITS-1:0] (dqs,     dqs_out_dly_bd,  dqs_out_en_dly_bd & {DQS_BITS{out_en}});
    bufif1 buf_dqs_n  [DQS_BITS-1:0] (dqs_n,   ~dqs_out_dly_bd, dqs_out_en_dly_bd & {DQS_BITS{out_en}});
    bufif1 buf_dq     [DQ_BITS-1:0]  (dq,      dq_out_dly_bd,   dq_out_en_dly_bd  & {DQ_BITS {out_en}});
...
`ifdef USE_TDQSCK_DLLDIS_RND
          seed=`SEED;
          tdqsck_dlloff_rnd = $dist_uniform(seed, TDQSCK_DLLDIS_MIN, TDQSCK_DLLDIS_MAX);
`else
          seed = RANDOM_SEED;
`endif
...
//        $display ("truebl4 = %d, relationship = %d, fromcmd = %h, cmd = %h", truebl4, relationship, fromcmd, cmd);
        casex ({truebl4, relationship, fromcmd, cmd})
	      ...
	      begin if ($time - tm_precharge < TRP)                                                                                   $display ("%m: at time %t ERROR:   tRP violation during %s", $time, cmd_string[cmd]);                         end
            ...
            {1'bx, DIFF_BANK , ZQ       , SELF_REF } : begin
`ifndef CLK_FREQ_CHG
...
`endif
    ...
    task cmd_task;
        ...
        begin
            // tRFC max check
            if (!er_trfc_max && !in_self_refresh && check_rfc_max) begin
			      ...
			      mode_reg[0][8] <= #($rtoi(tck_avg)) 1'b0;
                    ...
                    NOP: begin
                        if (in_power_down) begin
                            ...
                            if (check_pd_max && ($time - tm_cke_cmd > TPD_MAX))
            ...
            if (diff_ck) begin
                for (i=0; i<64; i=i+1) begin
                    if (dq_in_valid && dll_locked && ($time - tm_dqs_neg[i] < $rtoi(TDSS*tck_avg)))
                        if (check_dqs_ck_setup_hold) begin 
                          $display ("%m: at time %t ERROR: tDSS violation on %s bit %d", $time, dqs_string[i/32], i%32);
                          $display ("%m: at time %t ERROR: tm_dqs_neg[%0d] = %0d", $time, i, tm_dqs_neg[i]);
                        end
                    if (check_write_dqs_high[i] && check_dqs_latch)
                ...
                for (i=0; i<64; i=i+1) begin
                    if (dll_locked && dq_in_valid) begin
                        ...
                        if ((tm_tdqss < tck_avg/2.0) && (tm_tdqss > TDQSS*tck_avg))
                            if (check_dqs_ck_setup_hold) $display ("%m: at time %t ERROR: tDQSS violation on %s bit %d", $time, dqs_string[i/32], i%32);
                    end
                    if (check_write_dqs_low[i] && check_dqs_latch)
                        $display ("%m: at time %t ERROR: %s bit %d latching edge required during the preceding clock period", $time, dqs_string[i/32], i%32);
                end
...
`endif
                ...
                if (dll_en)
                    ...
                    begin
`ifdef USE_TDQSCK_DLLDIS_MIN
                      tdqsck_dlloff = TDQSCK_DLLDIS_MIN;
`elsif USE_TDQSCK_DLLDIS_MAX
                      tdqsck_dlloff = TDQSCK_DLLDIS_MAX;
`elsif USE_TDQSCK_DLLDIS_RND
                      tdqsck_dlloff =  tdqsck_dlloff_rnd;
`else
                      tdqsck_dlloff = TDQSCK_DLLDIS;
`endif
                    out_delay = ($rtoi(tck_avg/2) > 50000) ? 0 : $rtoi(tck_avg/2) + tdqsck_dlloff;
                    end
    ...
    always @ (posedge rst_n_in) begin : reset
        integer i;
        if (rst_n_in) begin
            if (check_clocks) begin
`ifdef FULL_SDRAM_INIT
            if ($time < 200000000 && check_strict_timing) 
                $display ("%m at time %t WARNING: 200 us is required before RST_N goes inactive.", $time);
`endif
                if (cke_in !== 1'b0 && $time > 0.001)
                    $display ("%m: at time %t ERROR: CKE must be inactive when RST_N goes inactive.", $time);
                if ($time - tm_cke < 10000 && $time > 5000)
...
`endif
    ...
    always @(negedge rst_n_in or posedge diff_ck or negedge diff_ck) begin : main
        ...
        if (!rst_n_in) begin
            ...
            if (check_clocks)    $display ("%m: at time %t ERROR: CK and CK_N are not allowed to go to an unknown state.", $time);
            data_task;
                    ...
                    if (mr_chk == 0) begin
					    ...
					    if (check_strict_timing) begin 
                            case (cas_write_latency)
                                ...
                                9 : if ((tck_avg < 1070.0) || (tck_avg >= 1250.0)) $display ("%m: at time %t ERROR: CWL = %d is illegal @tCK(avg) = %f", $time, cas_write_latency, tck_avg);
                                10: if ((tck_avg < 938.0) || (tck_avg >= 1070.0)) $display ("%m: at time %t ERROR: CWL = %d is illegal @tCK(avg) = %f", $time, cas_write_latency, tck_avg);
		    ...
		    if ((ck_cntr - ck_bank_write[i] > write_latency) && (ck_cntr - ck_bank_write[i] <= write_latency + burst_length/2)) begin
                ...
                // clk pin is disabled during self refresh
                if (!in_self_refresh) begin
                    if (dll_locked  && check_clocks && check_strict_timing) begin
            ...
            if (odt_en || dyn_odt_en) begin
                ...
                if (!in_self_refresh && diff_ck) begin
                    if ($time - tm_odt < TIS)
                        if (check_clocks)  $display ("%m: at time %t ERROR: tIS violation on ODT by %t", $time, tm_odt + TIS - $time);
...
`endif
                        ...
                        if (check_odth && !odt_in && (ck_cntr - ck_odt < ODTH4))
                            $display ("%m: at time %t ERROR:  ODTH4 violation during ODT transition", $time);
                        if (check_odth && !odt_in && (ck_cntr - ck_odth8 - ODTH8 < 0))
        ...
        if (rst_n_in) begin
            if ($time > TIH) begin
                if ($time - tm_ck_pos < TIH) 
                    if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIH violation on CKE by %t", $time, tm_ck_pos + TIH - $time);
            end
            if ($time - tm_cke < TIPW)
                    if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIPW violation on CKE by %t", $time, tm_cke + TIPW - $time);
        end
        tm_cke = $time;
    end
    always @(odt_in) begin
        if (rst_n_in && odt_en && !in_self_refresh) begin
            if ($time - tm_ck_pos < TIH) 
                if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIH violation on ODT by %t", $time, tm_ck_pos + TIH - $time);
            if ($time - tm_odt < TIPW)
                if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIPW violation on ODT by %t", $time, tm_odt + TIPW - $time);
        end
        tm_odt = $time;
    end
    ...
    begin
        if (rst_n_in && prev_cke) begin
            if ((i == 0) && ($time - tm_ck_pos < TIH))	               // always check tIH for CS#
                if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIH violation on %s by %t", $time, cmd_addr_string[i], tm_ck_pos + TIH - $time);
            if ((i > 0) && (cs_n_in == 0) &&($time - tm_ck_pos < TIH)) // Only check tIH for cmd_addr if CS# is low
                if (check_cmd_addr_timing) $display ("%m: at time %t ERROR:  tIH violation on %s by %t", $time, cmd_addr_string[i], tm_ck_pos + TIH - $time);
            if ((i == 0) && ($time - tm_cmd_addr[i] < TIPW))           // always check tIPW for CS#
                if (check_ctrl_addr_pulse_width) $display ("%m: at time %t ERROR: tIPW violation on %s by %t", $time, cmd_addr_string[i], tm_cmd_addr[i] + TIPW - $time);
    ...
    begin
        if (dqs_in_valid) begin
            if ($time - tm_dqs[i] < TDH) 
                if (check_dq_setup_hold) $display ("%m: at time %t ERROR:   tDH violation on DM bit %d by %t", $time, i, tm_dqs[i] + TDH - $time);
            if (check_dm_tdipw[i]) begin
                if ($time - tm_dm[i] < TDIPW)
                    if (check_dq_pulse_width) $display ("%m: at time %t ERROR: tDIPW violation on DM bit %d by %t", $time, i, tm_dm[i] + TDIPW - $time);
            end
        end
    ...
    begin
        if (dqs_in_valid) begin
            if ($time - tm_dqs[i/`DQ_PER_DQS] < TDH) 
                if (check_dq_setup_hold) $display ("%m: at time %t ERROR:   tDH violation on DQ bit %d by %t", $time, i, tm_dqs[i/`DQ_PER_DQS] + TDH - $time);
            if (check_dq_tdipw[i]) begin
                if ($time - tm_dq[i] < TDIPW)
                   if (check_dq_pulse_width)  $display ("%m: at time %t ERROR: tDIPW violation on DQ bit %d by %t", $time, i, tm_dq[i] + TDIPW - $time);
            end
        end
...
//                $display ("%m: at time %t WARNING: tWLS violation on DQS bit %d positive edge.  Indeterminate CK capture is possible.", $time, i);
        ...
        if (wl_feedback_on_all_bits) begin
          for (j=0; j<`DQ_PER_DQS; j=j+1) begin
          ...
          // Only on one particular bit
          for (j=0; j<`DQ_PER_DQS; j=j+1) begin
            if (j!= wl_feedback_on_bit) begin
              dq_out_en_dly[i*`DQ_PER_DQS+j] <= #(TWLO + TWLOE) 1'b1;
              dq_out_dly[i*`DQ_PER_DQS+j]    <= #(TWLO + TWLOE) 1'b0;
            end
            else begin
              dq_out_en_dly[i*`DQ_PER_DQS + wl_feedback_on_bit] <= #(TWLO) 1'b1;
              dq_out_dly[i*`DQ_PER_DQS + wl_feedback_on_bit]    <= #(TWLO) diff_ck;
            end
          end
        end
        end
                ...
                if (dll_locked) begin
                    if (check_write_preamble[i]) begin
                        ...
                        if ($time - tm_dqs_neg[i] < $rtoi(TDQSL*tck_avg))
                            if (check_dq_pulse_width) $display ("%m: at time %t ERROR: tDQSL violation on %s bit %d", $time, dqs_string[i/32], i%32);
                    end
                end
                if ($time - tm_dm[i%32] < TDS) 
                    if (check_dq_setup_hold) $display ("%m: at time %t ERROR: tDS violation on DM bit %d by %t", $time, i,  tm_dm[i%32] + TDS - $time);
                if (!dq_out_en) begin
                    for (j=0; j<`DQ_PER_DQS; j=j+1) begin
                        if ($time - tm_dq[(i%32)*`DQ_PER_DQS+j] < TDS) 
                            if (check_dq_setup_hold) $display ("%m: at time %t ERROR: tDS violation on DQ bit %d by %t", $time, i*`DQ_PER_DQS+j, tm_dq[(i%32)*`DQ_PER_DQS+j] + TDS - $time);
            ...
            if (dqs_in[i] ^ prev_dqs_in[i]) begin
                if (dll_locked) begin
                    ...
                    if ($time - tm_ck_pos < $rtoi(TDSH*tck_avg))
                        if (check_dqs_ck_setup_hold) $display ("%m: at time %t ERROR: tDSH violation on %s bit %d", $time, dqs_string[i/32], i%32);
                end
                if ($time - tm_dm[i%32] < TDS) 
                    if (check_dq_setup_hold) $display ("%m: at time %t ERROR: tDS violation on DM bit %d by %t", $time, i,  tm_dm[i%32] + TDS - $time);
                if (!dq_out_en) begin
