...
module mobile_ddr2 (
    ...
    reg     [DM_BITS-1:0]   dm_wr_in;
    reg     [DQ_BITS-1:0]   dq_wr_in;
    reg     [DQS_BITS-1:0]  dqs_wr_in;
    reg     [DQS_BITS-1:0]  dqs_n_wr_in;
    // transmit
    reg                       dqs_out_en_bd;
    reg                       dqs_out_bd;
    reg                       dq_out_en_bd;
    reg      [DQ_BITS-1:0]   dq_out_bd;
   ...
   parameter MRRBIT = 1'b0;
    ...
    // rx
    wire                  [7:0] dm_in = dm_wr_in;
    wire                 [63:0] dq_in = dq_wr_in;
    wire                  [7:0] dqs_even = dqs_wr_in;
    wire                  [7:0] dqs_odd = dqs_n_wr_in;
    reg                   [7:0] dm_in_pos;
    reg                   [7:0] dm_in_neg;
    reg                  [63:0] dq_in_pos;
    reg                  [63:0] dq_in_neg;
    ...
    always @(dqs_out_en) dqs_out_en_bd <= #(rd_dly) dqs_out_en;
    always @(dqs_out)    dqs_out_bd    <= #(rd_dly) dqs_out;
    always @(dq_out_en)  dq_out_en_bd  <= #(rd_dly) dq_out_en;
    always @(dq_out)     dq_out_bd     <= #(rd_dly) dq_out;

    bufif1 buf_dqs    [DQS_BITS-1:0] (dqs,    {DQS_BITS{dqs_out_bd}}, dqs_out_en_bd);
    bufif1 buf_dqs_n  [DQS_BITS-1:0] (dqs_n, {DQS_BITS{~dqs_out_bd}}, dqs_out_en_bd);
    bufif1 buf_dq      [DQ_BITS-1:0] (dq,                  dq_out_bd,  dq_out_en_bd);
...
`endif
    ...
    `endif
     $display("[MOBILE_DDR2] rnd_tdqsck = %0d", rnd_tdqsck);
    `endif
    ...
    always @(ck   ) ck_in       <= #(wr_dly) ck   ;
    always @(ck_n ) ck_n_in     <= #(wr_dly) ck_n ;
    always @(cke  ) cke_in      <= #(wr_dly) cke  ;
    always @(cs_n ) cs_n_in     <= #(wr_dly) cs_n ;
    always @(ca   ) ca_in       <= #(wr_dly) ca   ;
    always @(dm   ) dm_wr_in    <= #(wr_dly) dm   ;
    always @(dq   ) dq_wr_in    <= #(wr_dly) dq   ;
    always @(dqs  ) dqs_wr_in   <= #(wr_dly) dqs  ;
    always @(dqs_n) dqs_n_wr_in <= #(wr_dly) dqs_n;
